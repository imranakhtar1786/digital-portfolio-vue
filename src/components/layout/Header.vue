  ...
  <header class="fixed left-0 top-0 z-40 w-full px-4 py-4 md:px-8 transition-all duration-300">
    ...
    >
      ...
      <div class="hidden lg:flex items-center gap-3">
        ...
        >
          <span class="text-[10px] text-zinc-400 uppercase tracking-widest font-mono"
            >CURRENCY:</span
          >
        ...
        >
          ...
          <span class="text-[10px] font-mono tracking-widest text-[#D4AF37]">{{
            activeLocation
          }}</span>
      ...
      <!-- Navigation Links -->
      <div
        class="hidden md:flex items-center gap-6 text-xs uppercase tracking-[0.2em] font-semibold text-zinc-400"
      >
        <RouterLink
          to="/"
          class="transition duration-300 hover:text-[#D4AF37]"
          :class="{ 'text-[#D4AF37] font-bold': route.path === '/' }"
          >Home</RouterLink
        >
        <RouterLink
          to="/about"
          class="transition duration-300 hover:text-[#D4AF37]"
          :class="{ 'text-[#D4AF37] font-bold': route.path === '/about' }"
          >About Us</RouterLink
        >
        <RouterLink
          to="/services"
          class="transition duration-300 hover:text-[#D4AF37]"
          :class="{ 'text-[#D4AF37] font-bold': route.path.startsWith('/services') }"
          >Services</RouterLink
        >
        <RouterLink
          to="/projects"
          class="transition duration-300 hover:text-[#D4AF37]"
          :class="{ 'text-[#D4AF37] font-bold': route.path.startsWith('/projects') }"
          >Projects</RouterLink
        >
        <RouterLink
          to="/estimator"
          class="transition duration-300 hover:text-[#D4AF37]"
          :class="{ 'text-[#D4AF37] font-bold': route.path === '/estimator' }"
          >Estimator</RouterLink
        >
        <RouterLink
          to="/pricing"
          class="transition duration-300 hover:text-[#D4AF37]"
          :class="{ 'text-[#D4AF37] font-bold': route.path === '/pricing' }"
          >Pricing</RouterLink
        >
      </div>
      ...
      >
        <div class="flex flex-col gap-3 text-sm uppercase tracking-[0.2em]">
          ...
          <RouterLink
            ...
            class="rounded-xl px-4 py-3 text-zinc-300 transition hover:bg-[#121216] hover:text-[#D4AF37]"
            @click="closeMenu"
            >Projects (11)</RouterLink
          >
          <RouterLink
            to="/estimator"
            class="rounded-xl px-4 py-3 text-zinc-300 transition hover:bg-[#121216] hover:text-[#D4AF37]"
            @click="closeMenu"
            >Cost Estimator</RouterLink
          >
          <RouterLink
            to="/pricing"
            class="rounded-xl px-4 py-3 text-zinc-300 transition hover:bg-[#121216] hover:text-[#D4AF37]"
